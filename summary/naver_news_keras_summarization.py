...
def save_train_data():
    ...
    input_fname = './naver/naver_news_economy_norm_sort.txt'
    # input_fname = './naver/test.txt'
    ...
    while (True):
        ...
        if line_array[1].strip() == "" or len(line_array[1]) > 500:
            continue
        stories.append({'story': line_array[1], 'highlights': line_array[0]})
        # print({'story': line_array[1], 'highlights': line_array[0]})
    dump(stories, open('./naver/naver_news_economy.pkl', 'wb'))
    # dump(stories, open('./naver/test.pkl', 'wb'))
...
# stories = load(open('./naver/test.pkl', 'rb'))
