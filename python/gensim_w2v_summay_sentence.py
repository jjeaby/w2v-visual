...
def sumy(text, LANGUAGE='english', COUNT=2):
    parser = PlaintextParser.from_string(" ".join(texts), Tokenizer(LANGUAGE))
...
if __name__ == '__main__':
    ...
    for index in range(0, 750):
        ...
        summay_text = sumy(texts, COUNT=2)

        print("=" * 10)
        print(summay_text)
        print("=" * 10)

        nn_word_list = kakao_postagger_nn_finder(summay_text)

        print(nn_word_list)

        word2vec_data.append(nn_word_list)
    ...
    similar = model.most_similar(positive=['모르쇠', '일관', '한국', '규제'], topn=10)
    print(similar)

    word2vec_plt_show(model)
